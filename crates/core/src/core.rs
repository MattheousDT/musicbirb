...
impl Musicbirb {
	/// Queues a single track by its ID.
	/// Note: In Rust, pass by value: `musicbirb.queue_track(TrackId(id))`.
	...
	/// Queues all tracks from an album.
	pub async fn queue_album(&self, id: AlbumId) -> Result<u32, MusicbirbError> {
	...
	/// Queues all tracks from a playlist.
	pub async fn queue_playlist(&self, id: PlaylistId) -> Result<u32, MusicbirbError> {
	...
	pub async fn play_track(&self, id: TrackId) -> Result<(), MusicbirbError> {
		let track = self.api.get_track(&id).await?;
		self.tx
	...
	pub async fn play_album(&self, id: AlbumId) -> Result<u32, MusicbirbError> {
		let tracks = self.api.get_album_tracks(&id).await?;
		let count = tracks.len();
		self.tx
	...
	pub async fn play_playlist(&self, id: PlaylistId) -> Result<u32, MusicbirbError> {
		let tracks = self.api.get_playlist_tracks(&id).await?;
		let count = tracks.len();
		self.tx
			.send(CoreMessage::ReplaceTracks(tracks))
